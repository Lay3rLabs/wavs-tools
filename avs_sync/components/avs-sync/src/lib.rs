...
use bindings::{
    export,
    wavs::worker::layer_types::{TriggerData, TriggerDataEvmContractEvent, WasmResponse},
    Guest, TriggerAction,
};
...
impl Guest for Component {
    fn run(action: TriggerAction) -> std::result::Result<Option<WasmResponse>, String> {
        ...
        block_on(async move {
            // Convert addresses from alloy-sol-types to alloy-primitives format
            let reader_addr = alloy_primitives::Address::from_slice(reader_address.0.as_slice());
            let writer_addr = alloy_primitives::Address::from_slice(writer_address.0.as_slice());
            let contracts = AvsContracts::new(&chain_name, reader_addr, writer_addr)?;
            ...
            eprintln!(
                "AVS sync completed: {} operators updated",
                sync_result.operators_updated.len()
            );
            ...
            Ok(Some(WasmResponse {
                ...
                ordering: Some(trigger_id.to::<u64>()),
            }))
        })
    }
}
...
async fn perform_avs_sync(
    ...
    let operators_to_update = if let Some(prev_hash) = &previous_snapshot_hash {
        ...
        current_operators
            .iter()
            .filter(|(_, quorum_stakes)| quorum_stakes.values().any(|&stake| stake > U256::ZERO))
...
async fn upload_snapshot_to_ipfs(
    snapshot: &OperatorStakeSnapshot,
    ipfs_url: &str,
    ...
    let mut file =
        File::open(&temp_path).map_err(|e| format!("Failed to open temp file: {}", e))?;
    let mut file_bytes = Vec::new();
    file.read_to_end(&mut file_bytes).map_err(|e| format!("Failed to read temp file: {}", e))?;
    ...
    let mut response =
        Client::new().send(request).await.map_err(|e| format!("Failed to send request: {}", e))?;
